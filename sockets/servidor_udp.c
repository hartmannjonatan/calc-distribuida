...
int main()
{
  ...
  while(1) {
    ...
    ssize_t n = recvfrom(server_sockfd, operation, (sizeof(char)*MAX_LEN_OPERATION) - 1, MSG_WAITALL, (struct sockaddr*)&client_address, &client_len);
    if (n < 0) {
    ...
    printf("Operação recebida do cliente %s:%d > %s\n", client_ip, ntohs(client_address.sin_port), operation);

    struct data_result resultado = calc_operation(operation);
    sendto(server_sockfd, (void*)&resultado, sizeof(resultado), MSG_CONFIRM, (const struct sockaddr*)&client_address, client_len);
  }
}
